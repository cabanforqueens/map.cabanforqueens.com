      ...
      return `
        ...
        <div class="type-event type-action">
          <ul class="event-types-list">
            <li class='tag-${item.event_type} tag'>${item.campaign ? item.campaign.replace(/\-/g, " ") : (item.event_type ? item.event_type.replace(/\-/g, " ") : 'Action')}</li>
          </ul>
    ...
    return {
      ...
      updateBounds: (bound1, bound2, filters) => {
        ...
        listContainer.selectAll("li.org-list-item")
          ...
          .enter()
          .append('li')
            .attr("class", (item) => item.event_type != 'group' ? 'org-list-item events event-obj' : `org-list-item group-obj ${window.slugify(item.supergroup)}`)
