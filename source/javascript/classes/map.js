...
const MapManager = (($) => {
  ...
  const renderEvent = (item, referrer = null, source = null) => {
    ...
    //RC: .replace(' ', 'T') is a hack
    let m = moment(new Date(item.start_datetime.replace(' ', 'T')));
    ...
    <div class='popup-item ${item.event_type} ${superGroup}' data-lat='${item.lat}' data-lng='${item.lng}'>
      <div class="type-event">
        <ul class="event-types-list">
          <li class="tag tag-${item.event_type}">${item.campaign ? item.campaign.replace(/\-/g, " ") : (item.event_type ? item.event_type.replace(/\-/g, " ") : 'Action')}</li>
        </ul>
  ...
  return (options) => {
    ...
    window.map = map = new mapboxgl.Map({
      ...
      center: [-3.0955578063334315, 41.59973365141525],
      zoom: 1.2,
      interactive: true
      // scrollZoom: false
    });
    ...
    // map.on('load', ()=> {
    //   console.log(map, map.featuresAt);
    //
    // });

    LANGUAGE = options.lang || 'en';

    // map.on('style.loaded', (event) => {
      if (options.onMove) {
        // for cursor
        map.on('mouseover', '')
        // For drag end
        map.on('dragend', (event) => {
          ...
          $("#myRange").val(map.getZoom());
        })
        .on('dblclick', (event) => {
          map.easeTo({center: event.lngLat, zoom: map.getZoom()+1});
        })
      }
    ...
    $("#myRange").val(map.getZoom());
    //Slider events...
    $(document).on("click", ".zm-butt.zm-in", () => {
      const value = parseFloat($("#myRange").val());
      $("#myRange").val(Math.floor(value) + 1);
      $("#myRange").trigger("change");
    });

    $(document).on("click", ".zm-butt.zm-out", () => {
      const value = parseFloat($("#myRange").val());
      $("#myRange").val(Math.floor(value) - 1);
      $("#myRange").trigger("change");
    });

    $(document).on("mouseup dragend", ".zm-butt", () => {
    });

    $("#myRange").on("input change", () => {
      map.setZoom(parseFloat($("#myRange").val()));
    });
    ...
    return {
      ...
      plotPoints: (list, hardFilters, groups) => {
              ...
              map.addLayer({
                ...
                "layout": {
                  ...
                  "icon-size": [
                      ...
                      ["zoom"],
                      5, /*  Zoom level start */
                      0.09, /*  Size on Zoom level 5 */
                      8, /*  resize ends on Zoom level 8 */
                      0.15 /*  scale at zoom level 8 */
                  ]
                }
              })
            });
        ...
        //TODO - Get all features within a click..
        // https://s3.us-east-2.amazonaws.com/win-dot-beto/Precincts_2018_GOTV_new.geojson
        map.on('load', () => {
          map.on('mousemove', function (e) {
              var features = map.queryRenderedFeatures(e.point, { layers: Object.keys(groups).map(i => window.slugify(i)) });
